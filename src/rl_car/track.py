...
def hitbox_from_image(image_path: FilePath, hit_box_detail: float = 4.5) -> arcade.PointList:
    """Generate a valid hitbox from a given image file."""
...
def align_hitbox(
    hitbox: arcade.PointList, x_orig: int = WINDOW_WIDTH // 2, y_orig: int = WINDOW_HEIGHT // 2
) -> arcade.PointList:
    """Align a hitbox to a new origo, defaulting to the centre of the window."""
    return [(x + x_orig, y + y_orig) for x, y, *_ in hitbox]
